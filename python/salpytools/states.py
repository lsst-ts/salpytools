...
import logging
import inspect
from .utils import load_SALPYlib
from .state_transition_exception import StateTransitionException

__all__ = ['OfflineState', 'StandbyState', 'DisabledState', 'EnabledState', 'FaultState']
...
class DefaultState:
    """Base class for states in stateMachine.
    Loads SALPY_lib
    ...
    name: str
    subsystem_tag: str
        Name of the subsystem SALPY library to load.
    events: tuple of strings, opt
        Default = ('summaryState',)
    tsleep: float, opt
        Default 0.5
    """
    ...
    def subscribe_logEvent(self, eventname):
        ...
        self.myData[eventname] = getattr(self.SALPY_lib, '{}_logevent_{}C'.format(self.subsystem_tag,
                                                                                  eventname))()
